...
.js #loader { display: block; position: absolute; left: 100px; top: 0; }
    ...
    <body>
        ...
        <div class="container">
            <center><h2>Get your cakes from here!!</h2></center><hr><br>
            <div class="row">
                <?php
                ...
                <h3><?php echo $row['price']; ?>$<?php echo"&nbsp;".$row['category']; ?></h3>
                <a href="Cakes.php?cakeid=<?php echo $row['cakeid']; ?>&price=<?php echo $row['price']; ?>&buy='buy'"><input type="submit" class="btn btn-primary" name="buy" value="Buy"></a>
                <a href="Cakes.php?cakeid=<?php echo $row['cakeid']; ?>&price=<?php echo $row['price']; ?>&cart='cart'"><input type="submit" class="btn btn-primary" name="cart" value="Add to Cart"></a>
                </div>
                <?php
...
if(isset($_GET['buy']))
{
            ...
            $price = $_GET['price'];
            $stmt->execute();
            echo '<script LANGUAGE="JavaScript">alert("Your order will be delivered soon")</script>';
}
if(isset($_GET['cart']))
{
            $stmt = $connect->prepare("INSERT INTO tbl_cart_master (cust_id,cakeid,price) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $cust_id, $cakeid, $price);
            $cust_id = $_SESSION['custid'];
            $cakeid = $_GET['cakeid'];
            $price = $_GET['price'];
